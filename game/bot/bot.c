...
static void copy_level_shallow(Level* dst, const Level* src) {
    ...
    memcpy(dst->walls.data, src->walls.data,
           src->walls.words * sizeof(uint64_t));
    bb_init(&dst->boxes, src->boxes.rows, src->boxes.columns);

    memcpy(dst->boxes.data, src->boxes.data,
           src->boxes.words * sizeof(uint64_t));
    bb_init(&dst->goals, src->goals.rows, src->goals.columns);

    memcpy(dst->goals.data, src->goals.data,
           src->goals.words * sizeof(uint64_t));
    bb_init(&dst->player, src->player.rows, src->player.columns);

    memcpy(dst->player.data, src->player.data,
           src->player.words * sizeof(uint64_t));
}
...
static bool simple_deadlock(Level* lvl) {
    ...
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
            if (!bb_get(&lvl->boxes, r, c)) continue;

            if (bb_get(&lvl->goals, r, c)) continue;

            bool wall_up = (r == 0) ? true : bb_get(&lvl->walls, r - 1, c);
            bool wall_down =
                (r == rows - 1) ? true : bb_get(&lvl->walls, r + 1, c);
            bool wall_left = (c == 0) ? true : bb_get(&lvl->walls, r, c - 1);
            bool wall_right =
                (c == cols - 1) ? true : bb_get(&lvl->walls, r, c + 1);
...
static int heap_pop(MinHeap* h) {
    ...
    while (1) {
        ...
        if (l < h->size &&
            h->nodes[h->data[l]].f < h->nodes[h->data[smallest]].f)
            smallest = l;

        if (r < h->size &&
            h->nodes[h->data[r]].f < h->nodes[h->data[smallest]].f)
            smallest = r;
...
char* solve_astar(Level* lvl) {
    Node* nodes = malloc(sizeof(Node) * MAX_STATES);
    int nodes_count = 0;
    ...
    nodes_count = 1;
    hs_add(visited, hash_state(&nodes[0].lvl));
    ...
    while (open->size > 0) {
        ...
        if (check_win_lvl(&cur.lvl)) {
            found_idx = cur_idx;
            break;
        }
        int py = -1, px = -1;
        bb_find_first_bit(&cur.lvl.player, &py, &px);

        for (int d = 0; d < 4; d++) {
            int nx = px + DX[d], ny = py + DY[d];
            if (nx < 0 || ny < 0 || nx >= cur.lvl.walls.columns ||
                ny >= cur.lvl.walls.rows)
                continue;

            if (bb_get(&cur.lvl.walls, ny, nx)) continue;

            bool next_is_box = bb_get(&cur.lvl.boxes, ny, nx);
            if (next_is_box) {
                int bx = nx + DX[d], by = ny + DY[d];
                if (bx < 0 || by < 0 || bx >= cur.lvl.walls.columns ||
                    by >= cur.lvl.walls.rows)
                    continue;

                if (bb_get(&cur.lvl.walls, by, bx) ||
                    bb_get(&cur.lvl.boxes, by, bx))
                    continue;
            }

            if (nodes_count >= MAX_STATES) continue;

            int nidx = nodes_count++;
            copy_level_shallow(&nodes[nidx].lvl, &cur.lvl);
            if (next_is_box) {
                int bx = nx + DX[d], by = ny + DY[d];
                bb_clear(&nodes[nidx].lvl.boxes, ny, nx);
                bb_set(&nodes[nidx].lvl.boxes, by, bx);
            }

            bb_clear(&nodes[nidx].lvl.player, py, px);
            bb_set(&nodes[nidx].lvl.player, ny, nx);
    ...
    for (int i = 0; i < nodes_count; i++) free_level_shallow(&nodes[i].lvl);
